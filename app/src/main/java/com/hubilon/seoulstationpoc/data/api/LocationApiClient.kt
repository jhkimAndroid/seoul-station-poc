...
class LocationApiClient {
    ...
    /**
     * BLE+WiFi RSSI 배열과 센서 FloatArray를 서버에 전송하고 예측 좌표를 반환한다.
     * sensorValues는 FingerprintBuilder.toSensorArray()가 반환한 값을 그대로 전달한다.
     * 총 피처 수 = bleWifiValues.size + sensorValues.size (fetchAps 응답 기준으로 가변).
     * 코루틴 취소 시 OkHttp call 도 즉시 중단된다.
     */
    suspend fun predict(bleWifiValues: IntArray, sensorValues: FloatArray): LocationResult =
        ...
        Log.d(TAG, "[predict] POST $ENDPOINT | BLE+WiFi=${bleWifiValues.size}개 sensor=${sensorValues.size}개 total=$totalSize | 앞5=$preview")
        if (sensorValues.isNotEmpty()) Log.d(TAG, "  sensor=${sensorValues.toList()}")
        ...
        cont.invokeOnCancellation {
            call.cancel()
            Log.d(TAG, "[predict] 취소 — 새 요청으로 교체됨")
        }

        call.enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                val elapsed = System.currentTimeMillis() - startMs
                Log.e(TAG, "[predict] 네트워크 오류 (${elapsed}ms) | ${e.javaClass.simpleName}: ${e.message}")
            ...
            override fun onResponse(call: Call, response: Response) {
                val elapsed = System.currentTimeMillis() - startMs
                try {
                    Log.d(TAG, "[predict] HTTP ${response.code} (${elapsed}ms)")
                    if (!response.isSuccessful) {
                        val msg = "서버 오류: HTTP ${response.code}"
                        Log.e(TAG, "[predict] error : $msg")
                    ...
                    Log.i(TAG, "[predict] lat=${result.lat}, lng=${result.lng} (${elapsed}ms)")
                    cont.resume(result)
                } catch (e: Exception) {
                    Log.e(TAG, "[predict] 응답 파싱 오류 (${elapsed}ms) | ${e.message}")
                    if (!cont.isCancelled) cont.resumeWithException(e)
                }
            }
        })
    ...
    suspend fun fetchAps(): List<ApEntry> = suspendCancellableCoroutine { cont ->
        Log.d(TAG, "[fetchAps] GET $APS_ENDPOINT")
        ...
        call.enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                Log.e(TAG, "[fetchAps] 네트워크 오류 | ${e.message}")
                if (!cont.isCancelled) cont.resumeWithException(e)
            }
            ...
            override fun onResponse(call: Call, response: Response) {
                try {
                    ...
                    Log.i(TAG, "[fetchAps] 총 ${entries.size}개 — BLE=$bleCount, WiFi=$wifiCount, Sensor=$sensorCount")
                    cont.resume(entries)
                } catch (e: Exception) {
                    Log.e(TAG, "[fetchAps] 파싱 오류 | ${e.message}")
                    if (!cont.isCancelled) cont.resumeWithException(e)
                }
            }
        })
